...
classifier = DecisionTreeClassifier(criterion='entropy', random_state = 0)  #entropy for higher information gain
...
"""
|61, 7|
|      |   ==> 61 + 29 correct predictions, (5 + 7 )  wrong predictions
|3, 29|
"""
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j,0], X_set[y_set == j,1], c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('classifier decision tree (Test set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()
